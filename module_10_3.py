...
Также после всех операций поставьте ожидание в 0.001 секунды, тем самым имитируя скорость выполнения
пополнения.
Метод take:
...
class Bank:
    ...
    def deposit(self):
        for _ in range(1, 101):
            add_num = random.randint(50, 500)
            if self.balance >= 500 and self.lock.locked():
                self.lock.release()
            self.balance += add_num
            print(f'Пополнение: {add_num}. Баланс: {self.balance}')
            time.sleep(0.01)

    def take(self):
        for _ in range(1, 101):
            ded_num = random.randint(50, 500)
            print(f'Запрос на {ded_num}')
            if self.balance >= ded_num:
                self.balance -= ded_num
                print(f'Снятие: {ded_num}. Баланс: {self.balance}')
            else:
